...
export function UploadScreen({ assessmentId }: { assessmentId: string }) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      router.push(
        `/exams/${assessmentId}/extracting?job=${jobId}&submission=${submissionId}`,
      );
    } catch (error) {
      setStarting(false);
  ...
  return (
    <div className="mx-auto flex w-full max-w-[680px] flex-col items-center px-4 py-8 sm:px-6 sm:py-12">
      {/* ---------------- headline ---------------- */}
      <h1 className="text-center text-[22px] font-bold leading-tight tracking-tight text-ink-900 sm:text-[38px]">
        Upload{' '}
        <span className="headline-underline text-brand-500">
          Question Paper &amp; Answer Sheets
        </span>
      </h1>
      <p className="mt-2 text-center text-[13px] text-ink-600 sm:text-[15px]">
      ...
      <button
        ...
        disabled={!canStart}
        onClick={handleStart}
        className="mt-7 inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-[13px] font-semibold transition-colors enabled:bg-ink-900 enabled:text-white enabled:hover:bg-ink-800 disabled:cursor-not-allowed disabled:bg-ink-200 disabled:text-ink-400"
      >
        {starting ? (
          <>
